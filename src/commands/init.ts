...
async function resolveWizardSelection(
  ...
  const defaults: WizardSelection = {
    ...
    setupCodexConfig: !options.skipCodexConfig,
  ...
  const setupCodexConfig = options.skipCodexConfig
    ? false
    : await promptBoolean('Generate Codex MCP config snippet?', defaults.setupCodexConfig);
...
function renderCodexMcpConfigSnippet(config: CollabConfig): string {
  return [
    ...
    .action(async (options: InitOptions, command: Command) => {
      ...
      await runOrchestration(
        ...
        [
          ...
          {
            id: 'codex-config',
            title: 'Generate Codex MCP snippet',
            recovery: [
              'Verify permissions in .collab directory.',
              'Run collab init --resume to regenerate Codex config snippet.',
            ],
            run: () => {
              if (!selections.setupCodexConfig) {
                context.logger.info('Skipping Codex config stage by user choice.');
                return;
              }

              const target = path.join(effectiveConfig.collabDir, 'codex-mcp-config.toml');
              context.executor.writeFile(target, renderCodexMcpConfigSnippet(effectiveConfig), {
                description: 'write codex mcp config snippet',
              });
            },
          },
          {
